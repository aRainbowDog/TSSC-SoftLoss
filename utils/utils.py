...
# 假设生成的视频和真实的视频都已加载到 generated_video 和 gt_video 中，维度为 [12, 256, 256, 3]
# generated_video 和 gt_video 的数据类型为 np.array，范围在 [0, 255]
def calculate_metrics(generated_video, gt_video):
    generated_video = (generated_video - generated_video.min())/(generated_video.max() - generated_video.min())
    gt_video = (gt_video - gt_video.min()) / (gt_video.max() - gt_video.min())
    # 初始化总误差
    total_mse, total_mae, total_psnr, total_ssim = 0, 0, 0, 0
    num_frames = generated_video.shape[0]

    for i in range(num_frames):
        # 取出每一帧
        gen_frame = generated_video[i]
        gt_frame = gt_video[i]

        # 计算 MSE
        mse = mean_squared_error(gt_frame, gen_frame)
        total_mse += mse

        # 计算 MAE
        mae = np.mean(np.abs(gt_frame - gen_frame))
        total_mae += mae

        # 计算 PSNR，设置 data_range=1
        psnr = peak_signal_noise_ratio(gt_frame, gen_frame, data_range=1)
        total_psnr += psnr

        # 计算 SSIM，设置 data_range=1
        ssim = structural_similarity(gt_frame, gen_frame, multichannel=True, data_range=1, win_size=5, channel_axis=-1)
        total_ssim += ssim
