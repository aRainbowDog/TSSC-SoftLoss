...
class MVIFBlock(nn.Module):
    """
    A MVIF block with adaptive layer norm zero (adaLN-Zero) conditioning.
    """
...
class MVIF(nn.Module):
    ...
    def __init__(
        ...
        self.blocks = nn.ModuleList([
            MVIFBlock(hidden_size, num_heads, mlp_ratio=mlp_ratio, mode=mode, num_frames=self.num_frames) for _ in range(depth)
        ])
    ...
    def forward_with_cfg(self, x, t, y, cfg_scale):
        """
        Forward pass of MVIF, but also batches the unconditional forward pass for classifier-free guidance.
        """
...
def MVIF_L_2(**kwargs):
    return MVIF(depth=28, hidden_size=1152, patch_size=2, num_heads=16, **kwargs)

def MVIF_S_2(**kwargs):
    return MVIF(depth=12, hidden_size=384, patch_size=2, num_heads=6, **kwargs)
