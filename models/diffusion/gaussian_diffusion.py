...
class GaussianDiffusion:
    ...
    def p_mean_variance(self, model, x, t, clip_denoised=True, denoised_fn=None, model_kwargs=None, raw_x=None, mask=None):
        ...
        if self.model_var_type in [ModelVarType.LEARNED, ModelVarType.LEARNED_RANGE]:
            assert model_output.shape == (B, C * 2, *x.shape[2:])     # b, c, f, h, w
    ...
    def training_losses(self, model, x_start, t, model_kwargs=None, noise=None, mask=None):
        ...
        """
        x_start = x_start.permute(0, 2, 1, 3, 4)   # latent: [b, f, c, h, w] ——>[b, c, f, h, w]
        if model_kwargs is None:
            model_kwargs = {}
        if noise is None:
            noise = th.randn_like(x_start)
        ...
        if self.loss_type == LossType.MSE or self.loss_type == LossType.RESCALED_MSE:
            ...
            if self.model_var_type in [
                ...
                B, C = x_t.shape[:2]
                model_output = model_output.permute(0, 2, 1, 3, 4)   # [b, c*2, f, h, w], 模型预测的是均值和方差
                ...
                terms["vb"] = self._vb_terms_bpd(                     # 计算变分下界损失
